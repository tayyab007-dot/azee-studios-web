...
function LazyPortfolioVideo({ url }: { url: string }) {
...
export function PortfolioGallery() {
  ...
  useEffect(() => {
    ...
    async function fetchAllPortfolio() {
      setLoading(true);
      try {
        const res = await fetch(`/api/portfolio?category=${encodeURIComponent(activeTab)}`);
  ...
  return (
    <section ref={sectionRef} id="portfolio" className="pt-8 md:pt-12 pb-4 relative overflow-hidden">
            ...
            <AnimatePresence mode="popLayout">
              {visibleAssets.map((asset) => (
                ...
                >
                  ...
                  >
                    {asset.type === "video" ? (
                      <LazyPortfolioVideo url={asset.url} />
                    ) : (
                      <img
                        src={asset.url}
